...
def send_file(odoo, file_path):
    ...
    if filetype and filetype[0] in inv_mime:
        ...
        saleorder = f.read()
        f.close()
        inv_b64 = base64.b64encode(saleorder)
        wiz_id = odoo.execute(
            'sale.order.import', 'create',
            {'saleorder_file': inv_b64, 'saleorder_filename': filename})
        logger.debug("sale.order.import wizard_id=%d", wiz_id)
        try:
            action = odoo.execute(
                'sale.order.import', 'create_sale_order_action', wiz_id)
            if action.get('res_id'):
                logger.info(
                    'Saler Order ID %d successfully created in Odoo',
                    action['res_id'])
                saleorder_ids.append(action['res_id'])
                return 'success'
            else:
...
def main(options, arguments):
    ...
    logger.info(
        'RESULT: %d sale order%s created in Odoo, %d sale order import failure%s.',
        len(saleorder_ids), len(saleorder_ids) > 1 and 's' or '',
        len(fail_files), len(fail_files) > 1 and 's' or '')
    logger.debug('IDs of created sale orders: %s', saleorder_ids)
    logger.debug('Fail sale order imports: %s', fail_files)


if __name__ == '__main__':
    usage = "Usage: mass_saleorder_import.py [options] directory1 directory2 "\
        "directory3 ..."
    ...
    description = "This script is designed for mass import of "\
        "PDF or XML sale order in Odoo. The OCA module sale_order_import "\
        "must be installed on Odoo together with the module(s) that add "\
        "support for the specific sale order format."
