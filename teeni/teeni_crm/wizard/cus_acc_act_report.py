...
class CusSalesReport(models.TransientModel):
    ...
    def run_process(self):
        ...
        if self.from_date:
            ...
            if self.customer_id:
                whr_qry2 += ''' and rp.id=''' + str(self.customer_id.id)
            query += '''
                    union all
                    select
        ...
        for line in query_res:
            ...
            if 'inv_id' in line:
                ...
                if invoice_id.payment_term_id:
                    pmt_term = self.env['account.payment.term'].search([('id', '=', invoice_id.payment_term_id.id)],
                                                                       limit=1).name
    ...
    def print_report(self):
        return self.env.ref('teeni_crm.action_customer_acc_act_report').report_action(self)
