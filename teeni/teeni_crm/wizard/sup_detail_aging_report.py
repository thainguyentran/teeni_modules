...
class AgingReport(models.TransientModel):
    _name = 'sup.detail.aging.report.wiz'
    ...
    rec_lines = fields.Many2many('sup.detail.aging.report.detail')

    def run_process(self):
        list_rec = []
        return_obj = self.env['sup.detail.aging.report.detail']
        filter_val = [('state', '=', 'open'), ('type', 'in', ['in_invoice', 'in_refund'])]
        if self.supplier_id:
            filter_val.append(tuple(['partner_id', '=', self.supplier_id.id]))
        if self.aging_date:
        ...
        for rec in inv:
            ...
            dic = {
                ...
                "advance": rec.partner_id.credit-rec.partner_id.debit,
        ...
        for rec in partner_list:
            ...
            if i==1:
                ...
                in_out_obj = self.env['sup.detail.aging.report.detail'].create(heading)
                list_rec.append(in_out_obj.id)
                return_obj += self.env['sup.detail.aging.report.detail'].browse(in_out_obj.id)
            print("Detail", detail)
            in_out_obj = self.env['sup.detail.aging.report.detail'].create(detail)
            list_rec.append(in_out_obj.id)
            return_obj += self.env['sup.detail.aging.report.detail'].browse(in_out_obj.id)
        #
    ...
    def print_report(self):
        # self.run_process()
        return self.env.ref('teeni_crm.action_supplier_detail_aging_report').report_action(self)


class ReportDetail(models.Model):
    _name = 'sup.detail.aging.report.detail'
