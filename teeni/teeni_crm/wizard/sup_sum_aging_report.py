...
class PurchaseReport(models.TransientModel):
    _name = 'sup.sum.aging.report.wiz'
    ...
    rec_lines = fields.Many2many('sup.sum.aging.report.detail')

    def run_process(self):
        list_rec = []
        return_obj = self.env['sup.sum.aging.report.detail']
        ...
        for rec in partner_list:
            ...
            if i==1:
                ...
                in_out_obj = self.env['sup.sum.aging.report.detail'].create(heading)
                list_rec.append(in_out_obj.id)
                return_obj += self.env['sup.sum.aging.report.detail'].browse(in_out_obj.id)
            in_out_obj = self.env['sup.sum.aging.report.detail'].create(detail)
            list_rec.append(in_out_obj.id)
            return_obj += self.env['sup.sum.aging.report.detail'].browse(in_out_obj.id)
        #
    ...
    def print_report(self):
        # self.run_process()
        return self.env.ref('teeni_crm.action_supplier_sum_aging_report').report_action(self)


class ReportDetail(models.Model):
    _name = 'sup.sum.aging.report.detail'
